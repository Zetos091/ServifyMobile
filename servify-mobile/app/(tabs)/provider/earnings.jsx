            ...
            <>
              <View style={styles.heroCard}>
                ...
                <Text style={styles.heroValue}>
                  ₱{earnings?.total_earnings ? Number(earnings.total_earnings).toLocaleString() : "0"}
                </Text>
                <Text style={styles.heroSub}>From {earnings?.total_completed ?? 0} completed jobs</Text>
              </View>

              <View style={styles.statsRow}>
                {[
                  { icon: CheckCircle, label: "Completed",  value: earnings?.total_completed ?? 0,                                                    color: COLORS.success },
                  { icon: Clock,       label: "Pending",    value: earnings?.pending_payout ?? 0,                                                     color: COLORS.warning },
                  { icon: DollarSign,  label: "This Month", value: `₱${earnings?.this_month ? Number(earnings.this_month).toLocaleString() : "0"}`,   color: COLORS.info },
                ].map((stat) => {
                ...
                earnings.transactions.map((tx) => (
                  <View key={tx.id} style={styles.txCard}>
                    <View style={styles.txLeft}>
                      <Text style={styles.txService} numberOfLines={1}>{tx.service_title || "Service"}</Text>
                      <Text style={styles.txDate}>{tx.booking_date}</Text>
                    </View>
                    <View style={styles.txRight}>
                      <Text style={styles.txAmount}>+₱{tx.total_price}</Text>
                      <View style={[styles.txBadge, { backgroundColor: COLORS.successLight }]}>
                        <Text style={[styles.txBadgeText, { color: COLORS.success }]}>Completed</Text>
                      </View>
                    </View>
                  </View>
                ))
...
const styles = StyleSheet.create({
  ...
  heroValue: { fontSize: 40, fontWeight: "900", color: COLORS.white, marginBottom: 4 },
  ...
  statIcon: { width: 36, height: 36, borderRadius: RADIUS.md, justifyContent: "center", alignItems: "center", marginBottom: 8 },
  ...
  txCard: { backgroundColor: COLORS.card, borderRadius: RADIUS.xl, padding: 16, flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 10, ...SHADOW.sm },
